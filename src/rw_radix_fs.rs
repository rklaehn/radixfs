...
pub struct RwRadixFs {
    inner: Arc<Mutex<Inner>>,
}

impl RwRadixFs {
    pub fn open(file: File) -> Result<RwRadixFs> {
    ...
    fn rename(
        ...
        if flags & libc::RENAME_EXCHANGE as u32 != 0 {
            ...
            if inode_attrs.kind == FileKind::Directory {
                let mut entries =
                    check_error!(reply, inner.get_directory_content(inode_attrs.inode));
            ...
            if new_inode_attrs.kind == FileKind::Directory {
                let mut entries =
                    check_error!(reply, inner.get_directory_content(new_inode_attrs.inode));
                entries.insert(b"..".to_vec(), (parent, FileKind::Directory));
                check_error!(
                    reply,
                    inner.write_directory_content(new_inode_attrs.inode, entries)
                );
            }
